/*
	The use of this class is deprecated. Use the multithreaded variant instead. 
*/
var playable = true;
MIDI.loadPlugin({
...
function canvasplayback(index){
	...
	if (window.navigator.userAgent.indexOf("Android") != -1 || window.navigator.userAgent.indexOf("iPhone") != -1 || window.navigator.userAgent.indexOf("iPad") != -1) {
		console.log("Mobile Platform Detected. TestBPM. ");
		if (bpm>200) Materialize.toast("Canvas " + index + " has a high BPM, which may affect performance on a mobile device. ", 5000);
	} else {
		console.log("Desktop Platform Detected. No need to TestBPM. ");
	}
